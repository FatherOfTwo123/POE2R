...
An external, **read-only map/radar overlay for Path of Exile 2**.

It attaches to the PoE2 client, reads game state directly out of process memory (no injection, no
hooks, no input), and draws a terrain + entity overlay on top of the game's map.

> ⚠️ **Use at your own risk.** This reads another process's memory. It does not send input to the
> game, but reading another process may still violate Path of Exile's Terms of Service and could put
> your account at risk. This is a personal/educational tool — you are responsible for how you use it.
...
- **Web dashboard** (`http://localhost:7777`, or **F12** in-game) — a local control panel: a
  ...
  (radar display toggles, monster HP-bar visibility, map calibration). Served same-origin only;
  setting/navigation writes are loopback-gated. Read endpoints: `GET /state`, `/entities`,
  `/landmarks`.
...
- Windows SmartScreen may warn about an unsigned exe (expected for a community tool) — "More info →
  Run anyway".
- Antivirus may flag it because it reads another process's memory; that's inherent to what the
  tool does.
...
Hotkeys: **F9** quits; **F12** opens the web dashboard; **F6** routes to the nearest landmark/POI
and **F7** clears routes; on the **Atlas** screen, **F10** over a tile sets a route START, again on
another tile sets the END (the shortest path through the atlas connection graph is drawn), and a third
press resets it. Map calibration and all other settings live in the dashboard (there are no projection
calibration hotkeys, to avoid accidental presses).
...
- `src/POE2Radar.Overlay` — the radar `.exe`: attaches, AOB-resolves the game roots, runs the tick
  loop, renders the Direct2D overlay, and serves the API. Fully read-only — sends no input.
