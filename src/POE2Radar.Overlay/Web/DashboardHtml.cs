...
/// never input/automation); the Filters tab manages watched/hidden lists via <c>/api/watched</c> /
...
<div class="shell">
  ...
  <div class="body">
    <aside>
      ...
      <div id="alertsSect" class="sect" hidden>Zone Alerts</div>
      <div id="alerts"></div>
      ...
      <section class="view" data-view="atlas" hidden>
        <div class="panel-grid">
          <div class="card" style="grid-column:1/-1">
            ...
            <div class="row"><div class="rl hint-row">Ring positions are computed automatically from your window size &mdash; no calibration needed. Hover a tile in-game and press <b>F10</b> to set the route <b>START</b>, again on another tile for the <b>END</b> (the shortest path through the atlas connection graph is drawn); a 3rd press clears it.</div></div>
          </div>
        </div>
      </section>
      ...
      <section class="view" data-view="settings" hidden>
        <div class="panel-grid">
          <div class="card">
            ...
            <div class="row"><div class="rl">Entity linger<small>world-ticks (~30/s) an off-screen entity keeps showing &mdash; smooths Breach/Delirium flicker; 0 = off</small></div>
              <input class="numin" type="number" step="1" min="0" max="600" data-set="entityLingerFrames"></div>
          </div>
          <div class="card">
...
/* ── settings tab (writes radar/visual via the loopback-gated /api/settings) ── */
...
// Zone Alerts: detected preloads (zone-content) + active ailments. Both are inert (empty) until their
// offsets/AOB patterns are validated on PoE2 (see Poe2.Preload / Poe2.Buffs); the strip then lights up.
async function refreshAlerts(){
  try{
    const [pre,buf]=await Promise.all([getJSON('/api/preloads'),getJSON('/api/buffs')]);
    const chip=(text,c)=>'<span style="display:inline-block;margin:3px 5px 0 0;padding:2px 8px;border:1px solid '
      +esc(c)+';border-radius:10px;font-size:11px;color:'+esc(c)+'">'+esc(text)+'</span>';
    const chips=[];
    (pre||[]).forEach(p=>chips.push(chip(p.name, p.color||'#9aa')));
    (buf||[]).filter(b=>b.ailment).forEach(b=>chips.push(chip(b.name+(b.charges>1?(' ×'+b.charges):''), '#e0506e')));
    const sect=$('#alertsSect'), box=$('#alerts');
    if(chips.length){ sect.hidden=false; box.innerHTML=chips.join(''); }
    else { sect.hidden=true; box.innerHTML=''; }
  }catch(e){}
}
...
refreshAlerts(); setInterval(refreshAlerts, 1500);
checkVersion();
</script>
</body>
</html>
