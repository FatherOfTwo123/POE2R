...
///   GET  /api/buffs               — the local player's active buffs/debuffs/ailments (name/time/charges)
///   GET  /api/preloads            — important preloads detected for the current area (zone-content alerts)
///   GET  /api/icons               — the icon library (name + viewBox + paths) for the dashboard pickers
///   GET  /api/settings            — current radar/visual settings 
...
public sealed class ApiServer : IDisposable
{
    ...
    public void Start()
    {
        _listener.Start();
        _running = true;
        var t = new Thread(Loop) { IsBackground = true, Name = "PoeMapViewer.Api" };
        t.Start();
    }
        ...
        switch (path)
        {
            ...
            case "/state":
            {
                ...
                Write(ctx, 200, JsonSerializer.Serialize(new
                {
                    ...
                    hpPct = s.HpPct, manaPct = s.ManaPct, player = new { x = s.Player.X, y = s.Player.Y },
                    entityCount = s.Entities.Count,
                    poiCount = s.Entities.Count(e => e.Poi),
                    landmarkCount = s.Landmarks.Count,
                    buffCount = s.Buffs.Count,
                    preloadCount = s.Preloads.Count,
                    counts,
                }, Json));
                break;
            }
            ...
            case "/api/preloads":
            {
                // Important preloads detected for the current area (zone-content alerts). Empty until the
                // File Root + AreaChangeCounter AOB patterns are rediscovered for PoE2 (see Poe2.Preload).
                var preloads = s.Preloads.Select(h => new { name = h.Name, color = h.Color, path = h.Path });
                Write(ctx, 200, JsonSerializer.Serialize(preloads, Json));
                break;
            }

            case "/api/buffs":
            {
                // Player buffs/debuffs/ailments. Offsets are GH2/unvalidated on PoE2 (see Poe2.Buffs);
                // this is empty until the layout is validated via Research --buffs.
                var buffs = s.Buffs.Select(b => new
                {
                    name = b.Name, timeLeft = b.TimeLeft, maxTime = b.MaxTime, charges = b.Charges,
                    ailment = b.IsAilment,
                });
                Write(ctx, 200, JsonSerializer.Serialize(buffs, Json));
                break;
            }
            ...
            case "/entities":
            {
                ...
                var list = q2
                    ...
                    .Select(e => new
                    {
                        ...
                        poi = e.Poi, iconName = e.IconName, iconComplete = e.IconComplete, opened = e.Opened, reaction = e.Reaction, friendly = e.IsFriendly, rarity = e.Rarity.ToString(),
                        mods = e.ModList, itemArt = e.ItemArt,
                        x = e.Grid.X, y = e.Grid.Y, hpCur = e.HpCur, hpMax = e.HpMax,
                        alive = e.HpMax <= 0 || e.HpCur > 0, stale = e.Stale,
    ...
    private object ReadSettings() => new
    {
        ...
        entityLingerFrames = _settings.EntityLingerFrames,
    ...
    private string[] ApplySettings(string body)
    {
        ...
        foreach (var p in root.EnumerateObject())
        {
            switch (p.Name)
            {
                ...
                case "entityLingerFrames" when TryInt(p.Value, out var n): _settings.EntityLingerFrames = Math.Clamp(n, 0, 600); applied.Add(p.Name); break;
                ...
                case "hpBarUnique" when TryBool(p.Value, out var b): _settings.HpBarUnique = b; applied.Add(p.Name); break;                // Whole-object writes (the dashboard re-POSTs the full sub-object on edit). Parsed,
...
public sealed record RadarState(
    ...
    string CharName,
    int CharLevel,
    IReadOnlyList<Poe2Live.Buff> Buffs,
    IReadOnlyList<PreloadCatalog.Hit> Preloads)
{
    public static readonly RadarState Empty =
        new(false, 0, 0, false, 0, System.Numerics.Vector2.Zero,
            Array.Empty<Poe2Live.EntityDot>(), Array.Empty<Poe2Live.Landmark>(), 100, 100, "", "", 0,
            Array.Empty<Poe2Live.Buff>(), Array.Empty<PreloadCatalog.Hit>());
}
