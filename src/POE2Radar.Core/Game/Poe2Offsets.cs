...
public static class Poe2
{
    ...
    /// <summary>Buffs component (name confirmed by the by-name ComponentLookUp resolve) — the entity's
    /// active buffs/debuffs/ailments. <see cref="StatusEffectList"/> is a StdVector of pointers to
    /// <see cref="StatusEffectStruct"/>; each struct's BuffDefinitionPtr → a BuffDefinitions.dat row
    /// whose +0x00 is a pointer to the buff name. (GH2 offsets — PoE1 lineage; the StdVector offset and
    /// the row→name hops are UNVALIDATED on PoE2 and likely drifted, like Life did. Validate/auto-locate
    /// via <c>POE2Radar.Research --buffs</c>, which scans the component for the real list offset.)</summary>
    public static class Buffs
    {
        public const int StatusEffectList = 0x160; // (GH2) StdVector of StatusEffectStruct* — unvalidated on PoE2
    }

    /// <summary>BuffDefinitions.dat row (pointed to by <see cref="StatusEffectStruct.BuffDefinitionPtr"/>).
    /// +0x00 is a pointer to the buff's internal name (UTF-16). (GH2; unvalidated on PoE2.)</summary>
    public static class BuffDefinition
    {
        public const int NamePtr = 0x00; // (GH2) → UTF-16 buff name (e.g. "frozen", "flask_effect_life")
    }
    ...
    public static class MinimapIcon
    {
        ...
        // Icon NAME (the dat-row icon type, e.g. "Waypoint"/"NPC"/"MapDevice"/"ExpeditionCraftingBenchAct1"):
        // the component holds a pointer at +0x20 to a MinimapIcons.dat row whose first field (+0x00) is the
        // UTF-16 icon name. Reading it classifies a POI exactly as the game does instead of guessing from
        // metadata. ✓ validated live 2026-06-09 (--minimap resolved all 14 hideout POIs: Waypoint, MapDevice,
        // StashPlayer, CraftingBenchIcon, HealingWell, MarakethSanctumLocker, NPC, …).
        public const int NameRowPtr = 0x20; // ✓ → MinimapIcons.dat row ptr; row +0x00 → UTF-16 name
    }
    ...
    /// <summary>
    /// Loaded-files table (preload alerts, A1). When you enter an area the client preloads the files
    /// for everything in it (boss/unique/mechanic art + metadata); matching the loaded-file paths
    /// against a curated list announces a zone's contents the moment you arrive. Mirrors GameHelper2's
    /// LoadedFiles: a static root pointer → an array of <see cref="RootObjectCount"/> std::buckets, each
    /// a StdVector of <see cref="FileNodeStride"/>-byte nodes whose <see cref="FileInfoPtr"/> field →
    /// a file-info struct with a name (StdWString) and the area-change index it was loaded in.
    ///
    /// <para>✓ validated live 2026-06-09 (<c>--preload</c>): both AOB patterns resolve (File Root +
    /// AreaChangeCounter) and the struct layout reads real file paths (Data/Balance/*.dat,
    /// Metadata/Items/*, Art/Models/*). The GH2 (PoE1) byte signatures + struct offsets carried over to
    /// PoE2 unchanged. The per-file <see cref="IgnoreFirstAreas"/> test separates the zone's content from
    /// the always-loaded base set (which keeps a low load-index). Re-validate with <c>--preload</c>.</para>
    /// </summary>
    public static class Preload
    {
        public const int RootObjectCount  = 0x10; // ✓ number of std::buckets in the root array
        public const int RootObjectStride = 0x38; // ✓ sizeof std::bucket — each bucket's first 24B is its StdVector
        public const int FileNodeStride   = 0x18; // ✓ per-file node = 3 pointers (FilesPointerStructure)
        public const int FileInfoPtr      = 0x08; // ✓ node +0x08 → file-info struct
        public const int FileNameStr      = 0x08; // ✓ file-info +0x08 → StdWString path
        public const int FileAreaChangeId = 0x40; // ✓ file-info +0x40 → int area-change index it loaded in
        public const int IgnoreFirstAreas = 2;    // ✓ skip base-set indices (≤2); zone content gets a higher index
    }
}
