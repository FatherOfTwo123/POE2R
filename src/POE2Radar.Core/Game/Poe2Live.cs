...
public sealed class Poe2Live
{
    ...
    public readonly record struct EntityDot(
        ...
        string IconName = "", bool Stale = false, IReadOnlyList<string>? Mods = null, string? ItemArt = null, bool ItemIdentified = true)
    {
    ...
    /// so the render thread can anchor the guidance line at the player's live feet (and height-correct dots)
    /// without the local player needing to be in the (world-rate) entity list.</summary>
    ...
    public List<EntityDot> Entities(nint areaInstance)
    {
        if (areaInstance != _entCacheKey)
        {
            ...
            _category.Clear(); _meta.Clear(); _iconAddr.Clear(); _iconName.Clear(); _rarity.Clear(); _mods.Clear(); _itemIdent.Clear(); _idAt.Clear();
        ...
        while (_entQueue.Count > 0 && _entVisited.Count < 200000)
        {
            ...
            var dot = new EntityDot(id, entity, g, wv, cat, meta, hpCur, hpMax,
                poi, ReadReaction(entity), rarity, opened, iconComplete, iconName, Mods: mods, ItemArt: itemArt, ItemIdentified: itemIdentified);
            dots.Add(dot);
    ...
    private void EvictEntity(nint entity)
    {
        ...
        _meta.Remove(entity); _iconAddr.Remove(entity); _iconName.Remove(entity); _rarity.Remove(entity); _mods.Remove(entity); _itemIdent.Remove(entity);
    }
