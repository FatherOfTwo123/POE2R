...
if (HasFlag(args, "--minimap"))
    return RunMinimap(process, reader);

if (HasFlag(args, "--buffs"))
    return RunBuffs(process, reader);

if (HasFlag(args, "--preload"))
    return RunPreload(process, reader, TryGetHexArg(args, "--file-root") ?? 0, TryGetHexArg(args, "--area-counter") ?? 0);
...
// ── Validate the MinimapIcon NAME chain (I3) against the live client ───────────────────────
// Walks awake entities, resolves the "MinimapIcon" component (validated by-name), and for each POI
// prints CompletedState (+0x10) + the resolved icon name via the GH2 chain (+0x20 → row → +0x00 →
// UTF-16). If the names come out blank/garbage, the +0x20 row pointer offset drifted on PoE2 — the
// hex window of the component's first 0x40 bytes is printed for the first few so the real pointer
// field can be eyeballed (look for an offset whose 8-byte value points to a ptr that points to a
// readable ASCII/UTF-16 name like "Waypoint"/"NPC"/"RewardChest…").
static int RunMinimap(ProcessHandle process, MemoryReader reader)
{
    var (_, _, ai, _) = ResolveChain(process, reader);
    if (ai == 0) { Console.Error.WriteLine("Could not resolve chain (in game?)."); return 1; }
    ...
    var queue = new Queue<nint>(); queue.Enqueue(SafePtr(reader, head + Poe2.StdMapNode.Parent));
    var visited = new HashSet<nint>();
    var found = 0; var named = 0; var dumped = 0;
    var hex = new byte[0x40];
    while (queue.Count > 0 && visited.Count < 200000)
    {
        ...
        var icon = ResolveComponentAddr(reader, entity, "MinimapIcon");
        if (icon == 0) continue;
        found++;

        reader.TryReadStruct<int>(icon + Poe2.MinimapIcon.CompletedState, out var completed);
        var row = SafePtr(reader, icon + Poe2.MinimapIcon.NameRowPtr);
        var namePtr = row == 0 ? 0 : SafePtr(reader, row);
        var name = namePtr == 0 ? "" : reader.ReadStringUtf16(namePtr, 128);
        if (!string.IsNullOrEmpty(name)) named++;

        var meta = ReadEntityMetadata(reader, entity);
        Console.WriteLine($"  id={id} completed={completed} name=\"{name}\"  {meta}");

        // Hex-dump the first few icon components so a drifted name-pointer offset can be recovered.
        if (string.IsNullOrEmpty(name) && dumped < 4 && reader.TryReadBytes(icon, hex) == hex.Length)
        {
            dumped++;
            Console.WriteLine($"    comp 0x{icon:X} first 0x40 bytes:");
            for (var o = 0; o < hex.Length; o += 8)
            {
                var v = BitConverter.ToInt64(hex, o);
                Console.WriteLine($"      +0x{o:X2}: 0x{v:X16}");
            }
        }
    }
    Console.WriteLine($"\nMinimapIcon components: {found}; with a resolved name: {named}.");
    if (found > 0 && named == 0)
        Console.WriteLine("All names blank → Poe2.MinimapIcon.NameRowPtr (+0x20) likely drifted on PoE2; " +
                          "use the hex windows above to find the pointer field that leads to a readable name.");
    return 0;
}

// ── Validate + AUTO-LOCATE the Buffs status-effect list (A2) on the live client ────────────
// Resolves the player's "Buffs" component (by-name), then scans the component for the StdVector of
// status-effect pointers: for each candidate offset it reads the vector, dereferences the first few
// entries as StatusEffectStruct, resolves each buff's name (BuffDefinitionPtr → +0x00 → UTF-16), and
// scores the offset by how many readable buff names it yields. Prints the ranked candidates and the
// buffs each finds, so the real Poe2.Buffs.StatusEffectList offset can be read straight off the output
// after a patch. (Run with some buffs active — e.g. a flask up, or stand in a damaging ground effect.)
static int RunBuffs(ProcessHandle process, MemoryReader reader)
{
    var (_, _, _, lp) = ResolveChain(process, reader);
    if (lp == 0) { Console.Error.WriteLine("Could not resolve chain (in game?)."); return 1; }

    var buffs = ResolveComponentAddr(reader, lp, "Buffs");
    if (buffs == 0) { Console.Error.WriteLine("Player has no 'Buffs' component (in game?)."); return 1; }
    Console.WriteLine($"Player Buffs component @ 0x{buffs:X}");

    string ResolveBuffName(nint sePtr)
    {
        if (!reader.TryReadStruct<StatusEffectStruct>(sePtr, out var se) || !se.LooksValid()) return "";
        var namePtr = SafePtr(reader, se.BuffDefinitionPtr + Poe2.BuffDefinition.NamePtr);
        return namePtr == 0 ? "" : reader.ReadStringUtf16(namePtr, 128);
    }

    // Scan candidate StdVector offsets in the component; rank by readable buff-name yield.
    var best = (-1, 0, new List<string>());
    Console.WriteLine("\nScanning component for the status-effect StdVector (offset : count : sample names)…");
    for (var off = 0x80; off <= 0x400; off += 8)
    {
        if (!reader.TryReadStruct<StdVector>(buffs + off, out var vec)) continue;
        var count = ((long)vec.Last - (long)vec.First) / 8;
        if (vec.First == 0 || count is <= 0 or > 256) continue;
        var names = new List<string>();
        for (long i = 0; i < count && names.Count < 12; i++)
        {
            var sePtr = SafePtr(reader, vec.First + (nint)(i * 8));
            if (sePtr == 0) continue;
            var n = ResolveBuffName(sePtr);
            if (!string.IsNullOrEmpty(n)) names.Add(n);
        }
        if (names.Count == 0) continue;
        Console.WriteLine($"  +0x{off:X3} : count={count} : {string.Join(", ", names.Take(6))}");
        if (names.Count > best.Item1) best = (names.Count, off, names);
    }

    Console.WriteLine();
    if (best.Item1 < 0)
    {
        Console.WriteLine("No status-effect list found. Make sure you have at least one buff active " +
                          "(flask up / damaging ground / aura), then re-run. If still nothing, the " +
                          "StatusEffectStruct field layout (BuffDefinitionPtr@+0x08 / name@+0x00) may have " +
                          "drifted — --dump a status-effect pointer to re-derive it.");
        return 0;
    }

    Console.WriteLine($"BEST: Poe2.Buffs.StatusEffectList = 0x{best.Item2:X} " +
                      $"(currently 0x{Poe2.Buffs.StatusEffectList:X}). Found {best.Item1} named buff(s):");
    foreach (var n in best.Item3) Console.WriteLine($"    {n}");
    if (best.Item2 != Poe2.Buffs.StatusEffectList)
        Console.WriteLine($"\n→ Update Poe2.Buffs.StatusEffectList from 0x{Poe2.Buffs.StatusEffectList:X} to 0x{best.Item2:X}.");
    else
        Console.WriteLine("\n→ Configured offset matches — Poe2.Buffs.StatusEffectList is correct.");
    return 0;
}

// ── Discover/validate + dump the preload (loaded-files) table (A1) ─────────────────────────
// Resolves the File Root + AreaChangeCounter global slots via their AOB patterns (or accepts them as
// --file-root / --area-counter hex args to bypass AOB), then dumps the files the client loaded for the
// CURRENT area + the curated catalog matches. Use it to (1) confirm the patterns/offsets work on PoE2,
// and (2) capture real per-area preload paths to build important_preloads.json. Read-only.
static int RunPreload(ProcessHandle process, MemoryReader reader, nint manualFileRoot, nint manualAreaCounter)
{
    nint fileRoot = manualFileRoot, areaChange = manualAreaCounter;
    ...
    if (fileRoot == 0)
        foreach (var pat in AobPatterns.FileRootRefs)
        {
            foreach (var slot in AobScanner.ScanForResolvedAddresses(process, reader, pat).Distinct())
                if (reader.TryReadStruct<nint>(slot, out var p) && (ulong)p is > 0x10000 and < 0x7FFFFFFFFFFF) { fileRoot = slot; break; }
            if (fileRoot != 0) break;
        }
    ...
    if (areaChange == 0)
        ...
        {
            foreach (var slot in AobScanner.ScanForResolvedAddresses(process, reader, pat).Distinct())
                if (reader.TryReadStruct<int>(slot, out var v) && v is > 0 and < 10_000_000) { areaChange = slot; break; }
            if (areaChange != 0) break;
        }

    Console.WriteLine($"File Root slot:         {(fileRoot == 0 ? "NOT FOUND" : $"0x{fileRoot:X}")}");
    Console.WriteLine($"AreaChangeCounter slot: {(areaChange == 0 ? "NOT FOUND" : $"0x{areaChange:X}")}");
    if (fileRoot == 0 || areaChange == 0)
    {
        Console.WriteLine("\nOne or both globals were not resolved. The seeded AOB patterns are PoE1 and almost");
        Console.WriteLine("certainly don't match PoE2 — rediscover them (GameOffsets StaticOffsetsPatterns has the");
        Console.WriteLine("HowToFindIt recipes; or use IDA), update AobPatterns.{FileRootRefs,AreaChangeCounterRefs},");
        Console.WriteLine("and re-run. Or pass known addresses directly:");
        Console.WriteLine("    --preload --file-root 0x<slot> --area-counter 0x<slot>");
        return 0;
    }

    var pr = new PreloadReader(reader, fileRoot, areaChange);
    Console.WriteLine($"AreaChangeCounter value: {pr.AreaChangeValue}");

    // DIAGNOSTIC first: sample raw files across all buckets (no current-area filter) to confirm the
    // struct layout reads real paths and to see which area-change indices exist.
    var (samples, counts) = pr.RawSample(60);
    var nonEmpty = counts.Count(c => c > 0);
    Console.WriteLine($"Buckets: {counts.Count} ({nonEmpty} non-empty); node counts: [{string.Join(", ", counts)}]");
    if (samples.Count > 0)
    {
        var byIdx = samples.GroupBy(s => s.AreaChangeId).OrderBy(g => g.Key)
            .Select(g => $"{g.Key}×{g.Count()}");
        Console.WriteLine($"Sampled {samples.Count} file(s). AreaChangeId histogram (sample): {string.Join(", ", byIdx)}");
        Console.WriteLine("Sample paths (areaChangeId : path):");
        foreach (var (ac, path) in samples.Take(25)) Console.WriteLine($"  {ac,4} : {path}");
    }
    else
    {
        Console.WriteLine("No readable file paths in any bucket → the Poe2.Preload struct layout " +
                          "(RootObjectStride / FileNodeStride / FileInfoPtr / FileNameStr) is wrong for PoE2. " +
                          $"--dump 0x{SafePtr(reader, fileRoot):X} to eyeball the first bucket and re-derive it.");
    }

    var files = pr.CurrentAreaFiles();
    Console.WriteLine($"\nFiles loaded for the CURRENT area (areaChangeId == {pr.AreaChangeValue}): {files.Count}");
    foreach (var f in files.OrderBy(x => x, StringComparer.Ordinal).Take(80)) Console.WriteLine($"  {f}");
    if (files.Count > 80) Console.WriteLine($"  … and {files.Count - 80} more");

    var hits = PreloadCatalog.Shared.Match(files);
    Console.WriteLine($"\nCatalog matches ({PreloadCatalog.Shared.Count} rules): {hits.Count}");
    foreach (var h in hits) Console.WriteLine($"  {h.Name}  ←  {h.Path}");
    Console.WriteLine("\n(Tip: move to a couple of different zones and re-run to capture real per-area paths " +
                      "for important_preloads.json. If the sample paths above are real but the current-area " +
                      "count is 0, the area-change-id match offset (FileAreaChangeId) may differ — compare the " +
                      "histogram against the live counter.)");
    return 0;
}
