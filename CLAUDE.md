...
Reads game state out of process (no injection) and draws an overlay. **Fully read-only — sends no
input to the game.** Forked from a PoE1 framework, since rewritten around the live PoE2 layout.
...
**No input. Read-only.** The overlay never sends keystrokes or mouse input to the game — no
`SendInput`, no automation. (An earlier auto-flask feature was removed; do not reintroduce input
without an explicit decision.) This keeps the tool a pure observer.
...
- `src/POE2Radar.Overlay` — tick loop, Direct2D overlay, HTTP API. The deliverable `.exe`. Read-only.
...
- `RadarApp.cs` — **two threads** (the render thread is never blocked by the heavy walk):
  - *Render loop* (`Tick`, ~`FpsCap` Hz): fast per-frame reads on its OWN reader stack (`_liveRender`) —
    live player/vitals/camera/map + HP-bar live pos — then draws from the lock-free published
    snapshots. Publishes `RadarState` for the API. (Read-only: NO input is ever sent — the upstream
    auto-flask feature is deliberately stripped on this fork.)
  - *World loop* (`WorldLoop`→`WorldTick`, ~30 Hz, own thread + reader `_live`): the heavy entity/terrain/
    ...
    the per-area preload-alert scan. Publishes an immutable `WorldSnapshot` (+ a separate `AtlasRender`
    bundle) the render thread reads lock-free (volatile reference swap, same idiom as `_state`).
  - Three INDEPENDENT reader stacks over the one `ProcessHandle` (RPM is concurrency-safe; the per-instance
    ...
    guard). `/state` exposes `worldMs`/`renderMs` timers.
...
- `Web/ApiServer.cs` — read-only HTTP API on `localhost:7777` (`/state`, `/entities` (incl. each
  entity's `iconName`/`stale`), `/landmarks`, `/api/buffs`, `/api/preloads`, `/api/icons` — the icon
  library for the dashboard's SVG-preview shape pickers).

**Extra read layers (some offset-pending — degrade to empty until validated):**
- *MinimapIcon name* (I3) — `Poe2Live.ReadMinimapIconName` reads the POI's dat-row icon name
...
- *Preload alerts* (A1) — `Core/Game/PreloadReader.cs` reads the loaded-files table for the current
  area; `PreloadCatalog` (embedded `important_preloads.json`) matches it to zone-content alerts.
  Needs two AOB patterns (`AobPatterns.FileRootRefs` / `AreaChangeCounterRefs`, both PoE1 placeholders)
  **rediscovered for PoE2** + the `Poe2.Preload` struct offsets validated; inert until then. Resolved
  best-effort in `Bootstrap.ResolvePreloadSlots`. Validate/dump: `--preload` (or
  `--preload --file-root 0x.. --area-counter 0x..` to bypass AOB).
...
`--minimap` (validate the MinimapIcon name chain), `--buffs` (auto-locate the Buffs status-effect
list + dump player buffs), `--preload` (resolve the File Root + AreaChangeCounter slots and dump the
current area's loaded files + catalog matches),
...
DevTree), `--atlas-probe` (one-shot ATLAS PROJECTION recovery/validation — run with the Atlas map
...
`+0x5A8`; brute-scans for both so it self-heals on drift — the basis for node-to-node atlas routing.
`--atlas-current`/`--atlas-findcur`/`--atlas-marker` discover the current-location "player icon" node).

**Atlas overlay projection** (✓ live, pan + zoom): atlas nodes are UiElements; a node's screen pos is
`screen = (UIscale × zoom) × relPos` — relPos `+0x118` (read live; PAN is baked in), zoom =
node/canvas scale `+0x130` (read live), UIscale = winH/1600, NO offset (canvas origin = screen 0,0). NOT a
perspective homography, and NO calibration — the whole projection is derived live from the window height
(`RadarApp.AtlasProjection`), resolution-correct everywhere. See `resources/atlas-research-notes.md` "FULLY SOLVED".

**Atlas point-to-point routing** (✓ live): the node canvas carries a connection-edge `StdVector`
(`Poe2.AtlasGraph.ConnectionsVec +0x5A8`, 20-byte edges of grid-coord src→dst); `Poe2Atlas` builds the
adjacency once per canvas and `FindPath` A*'s over it. **F10** over a tile cycles route START → END →
reset (`RadarApp.AtlasRoutePick`); the shortest path is drawn through the graph (straight-line fallback
if unconnected), keyed by grid coord so it survives pan/zoom + off-screen tiles. Marks/route freeze when
the atlas view is static (a view+inputs signature) so off-screen arrows don't jitter.

**Render-rate HP bars** (✓ live): which mobs get a bar + their style (rarity gate + rule resolve + packed
colours) are decided once per world tick (`RadarApp.BuildHpSpecs`); only each bar's live position + HP are
re-read every render frame (`Poe2Live.TryLiveBar`, two tiny reads via cached component addresses), so
50–100 bars track moving monsters smoothly without re-enumerating thousands of entities per frame.
